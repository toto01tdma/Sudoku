...
// Check if a number is valid in a specific position for Diagonal Sudoku
export function isValidMove(board: SudokuBoard, row: number, col: number, num: number): boolean {
  ...
  // DIAGONAL SUDOKU: Check main diagonal (top-left to bottom-right)
  if (row === col) {
  ...
  // DIAGONAL SUDOKU: Check anti-diagonal (top-right to bottom-left)
  if (row + col === size - 1) {
    for (let i = 0; i < size; i++) {
...
// Simple Diagonal Sudoku solver using backtracking
...
// Generate a complete valid Diagonal Sudoku board
...
// Generate a playable Diagonal Sudoku puzzle by removing numbers from a complete board
...
// Validate the entire Diagonal Sudoku board
...
// Helper function to check if a cell is on the main diagonal
export function isOnMainDiagonal(row: number, col: number): boolean {
  return row === col;
}

// Helper function to check if a cell is on the anti-diagonal
export function isOnAntiDiagonal(row: number, col: number, size: number): boolean {
  return row + col === size - 1;
}

// Helper function to check if a cell is on any diagonal
export function isOnDiagonal(row: number, col: number, size: number): boolean {
  return isOnMainDiagonal(row, col) || isOnAntiDiagonal(row, col, size);
}
