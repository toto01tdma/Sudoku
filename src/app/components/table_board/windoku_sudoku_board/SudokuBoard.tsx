...
import { isWindokuCell } from './SudokuGenerator';
...
export default function SudokuBoard({
  ...
  // Handle keyboard input with windoku validation
  ...
  const getCellClassName = (row: number, col: number) => {
    ...
    // Background colors based on cell type and windoku pattern
    if (isPreview || isPrint) {
      // Print/preview mode: show windoku pattern
      ...
      if (originalBoard[row][col] !== null) {
        // Initial numbers: check if it's a windoku cell
        if (isWindokuCell(row, col)) {
          className += 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 '; // Gray background for windoku initial numbers
        } else {
          className += 'bg-white dark:bg-white text-gray-800 dark:text-gray-800 '; // White background for regular initial numbers
        }
      } else {
        // User input cells: check if this is a windoku cell
        if (isWindokuCell(row, col)) {
          className += 'bg-gray-200 dark:bg-gray-700 '; // Gray background for windoku cells
        } else {
  ...
  return (
    <div className={`${isPreview ? 'mb-8' : 'mb-6'} mx-auto`}>
      ...
      {/* Windoku Sudoku Legend */}
      {showTitle && !isPrint && (
        <div className="mt-2 text-center">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Windoku Sudoku: Numbers must not repeat in the windowed regions (gray cells)
          </p>
        </div>
      )}
    </div>
